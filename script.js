...
function getWeatherData(city, unit, hourlyorWeek) {
  fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?lang=ru&unitGroup=metric&key=PMNXHVK4M4D3ABJ54QLE7DFC5&contentType=json`,
    {
      method: "GET",
      headers: {},
    }
    ...
    .then((data) => {
      ...
      rain.innerText = "Осадки - " + today.precip + "%";
...
function updateHumidityStatus(humidity) {
  if (humidity <= 30) {
    ...
    humidityStatus.innerText = "Средняя";
  } else {
...
function updateVisibiltyStatus(visibility) {
  if (visibility <= 0.03) {
    ...
    visibilityStatus.innerText = "Очень легкая дымка";
...
function updateAirQualityStatus(airquality) {
  if (airquality <= 50) {
    airQualityStatus.innerText = "Хоороший👌";
  } else if (airquality <= 100) {
    airQualityStatus.innerText = "Средний😐";
  } else if (airquality <= 150) {
    airQualityStatus.innerText = "Вредно для чувствительных групп😷";
  } else if (airquality <= 200) {
    airQualityStatus.innerText = "Вредный😷";
  } else if (airquality <= 250) {
    airQualityStatus.innerText = "Очень вредный😨";
  } else {
    airQualityStatus.innerText = "Опасный😱";
  }
}
